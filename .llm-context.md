...
`filter`, `map`, and `effect` support async strategy options:

- **sequential** (default): Process one at a time
- **concurrent-unordered**: Process all concurrently, emit as completed
- **concurrent-ordered**: Process all concurrently, maintain order
