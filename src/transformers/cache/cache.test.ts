...
describe("cache transformer", () => {
  ...
  describe("dropStrategy", () => {
    it("should drop oldest values when maxSize exceeded (default)", async () => {
      const stream = new Stream<number>();
      const cached = stream.pipe(cache({ maxSize: 3 }));
    ...
    it("should drop newest values when dropStrategy is 'newest'", async () => {
      const stream = new Stream<number>();
      const cached = stream.pipe(cache({ maxSize: 3, dropStrategy: "newest" }));
  ...
  describe("cache management", () => {
    ...
    it("should expose size", async () => {
      const stream = new Stream<number>();
      const cached = stream.pipe(cache({ maxSize: 10 }));
  ...
  describe("TTL (Time-To-Live)", () => {
    ...
    it("should work with both maxSize and TTL", async () => {
      const stream = new Stream<number>();
      const evicted: Array<{ value: number; reason: string }> = [];

      const cached = stream.pipe(cache({ maxSize: 2, ttl: 100 }));
      cached.cache.evicted.listen((e) => evicted.push(e));

      stream.push(1, 2, 3); // 1 evicted by maxSize
      expect(cached.cache.values).toEqual([2, 3]);
      expect(evicted).toEqual([{ value: 1, reason: "maxSize" }]);
  ...
  describe("evicted stream", () => {
    it("should emit eviction events when maxSize is exceeded", async () => {
      const stream = new Stream<number>();
      const evicted: Array<{ value: number; reason: string }> = [];

      const cached = stream.pipe(cache({ maxSize: 2 }));
      cached.cache.evicted.listen((e) => evicted.push(e));
    ...
    it("should work with dropStrategy newest", async () => {
      ...
      const cached = stream.pipe(cache({ maxSize: 2, dropStrategy: "newest" }));
      cached.cache.evicted.listen((e) => evicted.push(e));
      ...
      expect(evicted).toEqual([
        { value: 3, reason: "maxSize" },
        { value: 4, reason: "maxSize" },
      ]);
